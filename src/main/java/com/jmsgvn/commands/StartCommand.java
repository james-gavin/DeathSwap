...
public class StartCommand extends BaseCommand {
    ...
    public void start(Player player) {
        ...
        if (player.getServer().getOnlinePlayers().size() <= 1) {
            player.sendMessage(ChatColor.RED + "There are not enough players online to start the game.");
            return;
        }

        if(!DeathSwap.getInstance().getGame().getPhase().equals(GamePhase.LOBBY)) {
            player.sendMessage(ChatColor.RED + "There is already a game in session.");
            return;
        }
